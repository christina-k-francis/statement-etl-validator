...
Runs both stages end-to-end from a single YAML config file:
    Stage 1 - _00_extract_tables: PDF → numbered CSVs
    Stage 2 - _01_parse_holdings: CSVs → holdings.csv + initial_issue_report.txt
    Stage 3 - _02_llm_validation: LLM-assisted self-correcting validation (fixed holdings.csv + llm_validation_report.txt)

Usage:
    python pipeline.py <config.yaml>

Example:
    python pipeline.py Full_Pipeline/fidelity.yaml
...
def full_pipeline_flow(cfg: dict) -> None:
    ...
    Description:
        ...
        extraction_flow  →  parsing_flow → self_correcting_llm_validation_flow 
